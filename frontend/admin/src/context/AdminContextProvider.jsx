...
const AdminContextProvider = (props) => {
  ...
  const deleteShippingRates = async (shippingRateID) => {
    if (!token) return;
    try {
      const response = await axios.delete(
        backendUrl + "/api/location/shipping-rates/delete",
        {
          headers: { Authorization: `Bearer ${token}` },
          data: { shippingRateID }    // ✅ Correct way to send body in DELETE
        }
      );
  ...
  const updateShippingRates = async (payload) => {
    if (!token) return;
    try {
      const response = await axios.put(backendUrl + "/api/location/shipping-rates/update", payload, 
        { headers: { Authorization: `Bearer ${token}` } });
  ...
  /*---------------------------ADD SHIPPING RATES-----------------------------*/
  const addShippingRates = async (payload) => {
    if (!token) return;
    try {
      const response = await axios.post(backendUrl + "/api/location/shipping-rates/add", payload, 
        { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleFetchShippingRates = async () => {
    try {
      const response = await axios.get(backendUrl + "/api/location/shipping-rates/fetch", {
          headers: {
  ...
  const deleteInventorySettings = async (productInventorySettingsID) => {
    if (!token) return;
    try {
      const response = await axios.delete(backendUrl + "/api/inventory/settings/delete",
        {
          headers: { Authorization: `Bearer ${token}` },
          data: { productInventorySettingsID }
        }
      );
  ...
  const updateInventorySettings = async (payload) => {
    if (!token) return;
    try {
      const response = await axios.patch(backendUrl + "/api/inventory/settings/update", payload, 
        { headers: { Authorization: `Bearer ${token}` } });
  ...
  const addInventorySettings = async (payload) => {
    if (!token) return;
    try {
      const response = await axios.post(backendUrl + "/api/inventory/settings/add", payload, 
        { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleFetchInventorySettings = async () => {
    try {
        const response = await axios.get(backendUrl + "/api/inventory/settings/fetch", {
            headers: {
            Authorization: `Bearer ${token}`
            }
        });
  ...
  const adjustStock = async (payload) => {
    ...
    try {
      const response = await axios.post(
        backendUrl + "/api/inventory/adjust-stock",
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      console.error(error);
      toast.error("Error processing stock adjustment.", { ...toastError });
      return null;
    }
  };
  ...
  const processRefundStock = async (refundId, stockAction, quantity) => {
    if (!token) return;
    try {
      const response = await axios.patch(
        backendUrl + "/api/order/refund/process-stock",
        { refundId, stockAction, quantity },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.success) {
        toast.success(response.data.message, { ...toastSuccess });
  ...
  /*---------------------------DELETE REFUND REQUEST (ADMIN SOFT DELETE)-----------------------------*/
  const deleteRefundRequest = async (refundID) => {
    if (!token) return;
    try {
      const response = await axios.patch(
        backendUrl + "/api/order/refund/delete-record",
        { refundID },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.success) {
  ...
  /*---------------------------ADMIN DELETE ORDER ITEM (SOFT DELETE)-----------------------------*/
  const adminDeleteOrderItem = async (orderItemId) => {
    if (!token) return;
    try {
      const response = await axios.patch(
        backendUrl + "/api/order/cancel/delete-order-item",
        { orderItemId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.success) {
  ...
  const adminRemoveCancellation = async (orderCancelID, customerID, orderItemID) => {    if (token) {
