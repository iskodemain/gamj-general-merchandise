...
const Product = () => {
  ...
  const { products, productVariantValues, variantName, currency, addToCart, toastError, navigate, addToWishlist, removeFromWishlist, isInWishlist, cartItems, showUnavailableNote, setShowUnavailableNote, verifiedUser, productVariantCombination, token, setOrderItems, getOrderSubTotal, fetchInventoryStock} = useContext(ShopContext);
  ...
  function getStockFromInventory(productId, variantValueId = null, variantCombinationId = null) {
    return (fetchInventoryStock.find(s =>
      s.productId === productId &&
      (variantValueId !== null ? s.variantValueId === variantValueId : s.variantValueId === null) &&
      (variantCombinationId !== null ? s.variantCombinationId === variantCombinationId : s.variantCombinationId === null)
    )?.totalQuantity) ?? 0;
  }
  ...
  const fetchProductData = async () => {
    try {
      ...
      if (product) {
        ...
        const baseStock = getStockFromInventory(product.ID);
  ...
  const handleVariantSelect = (variantType, ID) => {
    ...
    if (!productData.hasVariant && !productData.hasVariantCombination) {
      ...
      const productStock = getStockFromInventory(productData.ID);
    ...
    if (productData.hasVariant && !productData.hasVariantCombination) {
      ...
      if (match) {
        ...
        const productStock = getStockFromInventory(productData.ID, match.ID);
        ...
        setStock(getStockFromInventory(productData.ID));
      }
      return;
    }
