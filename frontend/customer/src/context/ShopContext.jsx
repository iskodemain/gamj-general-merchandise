...
const ShopContextProvider = (props) => {
    ...
    const [rejectedCustomer, setRejectedCustomer] = useState(null);
    ...
    const [showOrderProofPayment, setShowOrderProofPayment] = useState(false);
    ...
    const editOrderProofPayment = async(paymentProofID, paypalAccountName, referenceId, receiptImage) => {
        if (token) {
            try {
                ...
                const response = await axios.put(backendUrl + "/api/order/payment-proof/edit", formData, {
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    /*--------------------------ADD ORDER REFUND----------------------------*/
    const addOrderRefund = async (orderItemId, reasonForRefund, refundComments, imageProof1, imageProof2, refundResolution, otherReason, refundMethod, refundPaypalEmail, refundStatus, returnQuantity, returnMethod) => {
        if (token) {
            try {
                const formData = new FormData();
                ...
                formData.append('returnQuantity', returnQuantity || 1);
    ...
    /*--------------------------ADD ORDER----------------------------*/
    const addOrder = async (paymentMethod, orderItems, cartItemsToDelete, subtotal, shippingFee, totalAmount, paypalFee = null) => {
        if (token) {
            try {
